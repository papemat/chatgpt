...
from core.config import TokIntelConfig
...
# TEST: edge-case - Input vuoto
def test_analyze_engagement_empty_input():
    from agent.synthesis import SynthesisAgent
    agent = SynthesisAgent(model="gpt-4", endpoint="", api_key="test")
    with pytest.raises(ValueError):
        agent.analyze_engagement_factors("", "", {})

# TEST: edge-case - Input rumoroso o troppo lungo
def test_analyze_engagement_noisy_input():
    from agent.synthesis import SynthesisAgent
    agent = SynthesisAgent(model="gpt-4", endpoint="", api_key="test")
    noisy_text = "!!!####aaaaa" * 1000
    try:
        result = agent.analyze_engagement_factors(noisy_text, noisy_text, {})
        assert result is not None
    except Exception as e:
        assert "token" in str(e).lower() or "length" in str(e).lower() or "encoding" in str(e).lower()

# TEST: edge-case - Categoria sconosciuta
def test_scoring_unknown_category(monkeypatch):
    from agent.synthesis import SynthesisAgent
    agent = SynthesisAgent(model="gpt-4", endpoint="", api_key="test")
    def fake_call_openai_llm(*args, **kwargs):
        return '{"unknown_category": 42}'
    monkeypatch.setattr(agent, "_call_openai_llm", fake_call_openai_llm)
    result = agent.analyze_engagement_factors("test", "test", {})
    assert "unknown_category" in str(result)
