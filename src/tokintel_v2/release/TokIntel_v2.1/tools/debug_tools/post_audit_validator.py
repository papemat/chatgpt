...
class PostAuditValidator:
    ...
    def print_summary(self):
        ...
        logger.info(f"f"Python files validated: {len(self.python_files")}")
        logger.info(f"f"Total issues found: {total_issues}"")
        
        for category, issues in self.report.items():
            if issues:
                logger.info(f"f"\n{category.upper(").replace('_', ' ')} ({len(issues)}):")
                for issue in issues[:3]:  # Show first 3
                    logger.info(f"f"  - {issue}"")
                if len(issues) > 3:
                    logger.info(f"f"  ... and {len(issues") - 3} more")
